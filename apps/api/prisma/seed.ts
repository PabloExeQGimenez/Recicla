...
async function main() {
  ...
  const pesajesTemplates = [
    ...
    { recuperadorIdx: 0, items: [{ materialIdx: 2, minW: 18, maxW: 45 }, { materialIdx: 4, minW: 12, maxW: 30 }] },
    { recuperadorIdx: 1, items: [{ materialIdx: 3, minW: 22, maxW: 55 }] },
    { recuperadorIdx: 2, items: [{ materialIdx: 6, minW: 15, maxW: 40 }, { materialIdx: 8, minW: 10, maxW: 25 }] },
    { recuperadorIdx: 3, items: [{ materialIdx: 1, minW: 28, maxW: 65 }] },
    { recuperadorIdx: 4, items: [{ materialIdx: 7, minW: 10, maxW: 30 }, { materialIdx: 9, minW: 15, maxW: 40 }] },
    { recuperadorIdx: 5, items: [{ materialIdx: 0, minW: 20, maxW: 50 }, { materialIdx: 2, minW: 12, maxW: 35 }] },
    { recuperadorIdx: 0, items: [{ materialIdx: 5, minW: 16, maxW: 40 }] },
    { recuperadorIdx: 1, items: [{ materialIdx: 4, minW: 25, maxW: 60 }, { materialIdx: 6, minW: 14, maxW: 35 }] },
    { recuperadorIdx: 2, items: [{ materialIdx: 3, minW: 18, maxW: 45 }] },
    { recuperadorIdx: 3, items: [{ materialIdx: 8, minW: 12, maxW: 30 }, { materialIdx: 9, minW: 20, maxW: 50 }] },
  ];

  const statusDistribution: PesajeStatus[] = [
    ...Array(15).fill(PesajeStatus.PENDING),
    ...Array(13).fill(PesajeStatus.PAYMENT_REQUESTED),
    ...Array(12).fill(PesajeStatus.PAID),
  ];
  ...
  const augustPesajes = allPesajes.filter(
    (p) => p.date.getMonth() === 7,
  );

  const septemberPesajes = allPesajes.filter(
    (p) => p.date.getMonth() === 8,
  );

  if (augustPesajes.length >= 2) {
    const sortedAsc = [...augustPesajes].sort(
    ...
    const solicitud1 = await prisma.solicitudPago.create({
      data: {
        ...
        to: sortedAsc[sortedAsc.length - 1].date,
        status: SolicitudPagoStatus.PAYMENT_REQUESTED,
        pesajes: {
          connect: sortedAsc.slice(0, Math.min(5, sortedAsc.length)).map(p => ({ id: p.id })),
        },
      },
    });
    console.log(`Solicitud de pago #1 creada: Agosto 2026 (PAYMENT_REQUESTED)`);
  }

  if (septemberPesajes.length >= 2) {
    const sortedAsc = [...septemberPesajes].sort(
      (a, b) => a.date.getTime() - b.date.getTime(),
    );
    ...
    console.log(`Solicitud de pago #2 creada: Septiembre 2026 (PAID)`);
  }

  if (paymentRequestedPesajes.length >= 4) {
    const remaining = paymentRequestedPesajes.slice(5, 9);
    if (remaining.length >= 2) {
      ...
      const solicitud3 = await prisma.solicitudPago.create({
        data: {
          ...
          status: SolicitudPagoStatus.PAYMENT_REQUESTED,
          pesajes: {
            connect: sortedAsc.slice(0, Math.min(4, sortedAsc.length)).map(p => ({ id: p.id })),
          },
        },
      });
      console.log(`Solicitud de pago #3 creada: Agosto-Septiembre 2026 (PAYMENT_REQUESTED)`);
    }
  }
