...
describe('AuthService', () => {
  ...
  describe('login', () => {
    it('debería lanzar UnauthorizedException si el usuario no existe', async () => {
      ...
      await expect(service.login('test@test.com', 'pass')).rejects.toThrow('Credenciales inválidas');
    });

    it('debería lanzar UnauthorizedException si la contraseña es incorrecta', async () => {
      mockPrisma.user.findUnique.mockResolvedValue({
        id: 'u-1', email: 'test@test.com', password: 'hash', role: 'OPERADOR',
      });
    ...
    it('debería retornar token y usuario en login exitoso', async () => {
      const user = {
        id: 'u-1', name: 'Juan', lastName: 'Pérez', dni: '12345678',
        email: 'test@test.com', password: 'hash', role: 'OPERADOR',
      };
      ...
      expect(mockJwt.sign).toHaveBeenCalledWith({
        sub: 'u-1', email: 'test@test.com', role: 'OPERADOR',
      });
    });
  });
  ...
  describe('register', () => {
    ...
    it('debería lanzar ConflictException si el DNI ya existe', async () => {
      ...
      await expect(
        service.register('Juan', 'Pérez', 'new@test.com', 'pass123', 'OPERADOR', '12345678'),
      ).rejects.toThrow(ConflictException);
      await expect(
        service.register('Juan', 'Pérez', 'new@test.com', 'pass123', 'OPERADOR', '12345678'),
    ...
    it('debería crear usuario y retornar token', async () => {
      ...
      mockPrisma.user.create.mockResolvedValue({
        id: 'u-1', name: 'Juan', lastName: 'Pérez', dni: null,
        email: 'new@test.com', role: 'OPERADOR',
      });
      mockJwt.sign.mockReturnValue('jwt-token');

      const result = await service.register('Juan', 'Pérez', 'new@test.com', 'pass123');
    ...
    it('debería crear usuario con DNI si se provee', async () => {
      ...
      const result = await service.register(
        'Juan', 'Pérez', 'new@test.com', 'pass123', 'ADMIN', '12345678',
      );
  ...
  describe('getProfile', () => {
    it('debería lanzar UnauthorizedException si el usuario no existe', async () => {
      ...
      await expect(service.getProfile('u-1')).rejects.toThrow(UnauthorizedException);
    });

    it('debería retornar el perfil del usuario', async () => {
      const user = { id: 'u-1', name: 'Juan', lastName: 'Pérez', dni: '12345678', email: 'test@test.com', role: 'OPERADOR' };
      mockPrisma.user.findUnique.mockResolvedValue(user);
  ...
  describe('findAll', () => {
    it('debería retornar todos los usuarios', async () => {
      const users = [
        { id: 'u-1', name: 'Juan', lastName: 'Pérez', dni: null, email: 'a@test.com', role: 'OPERADOR', createdAt: new Date() },
      ];
      ...
      expect(mockPrisma.user.findMany).toHaveBeenCalledWith({
        select: { id: true, name: true, lastName: true, dni: true, email: true, role: true, createdAt: true },
  ...
  describe('findById', () => {
    ...
    it('debería permitir a un OPERADOR ver su propio perfil', async () => {
      const user = { id: 'u-1', name: 'Juan', lastName: 'Pérez', dni: null, email: 'test@test.com', role: 'OPERADOR', createdAt: new Date() };
      mockPrisma.user.findUnique.mockResolvedValue(user);

      const result = await service.findById('u-1', { id: 'u-1', role: 'OPERADOR' });

      expect(result.id).toBe('u-1');
    });

    it('debería permitir a un ADMIN ver cualquier usuario', async () => {
      const user = { id: 'u-2', name: 'María', lastName: 'García', dni: null, email: 'other@test.com', role: 'OPERADOR', createdAt: new Date() };
      mockPrisma.user.findUnique.mockResolvedValue(user);

      const result = await service.findById('u-2', { id: 'u-1', role: 'ADMIN' });
