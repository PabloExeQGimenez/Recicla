...
describe('MaterialController', () => {
  ...
  const material = new Material({
    id: 'm-1', name: 'Cartón', currentPrice: 2, active: true,
  ...
  describe('create', () => {
    it('debería crear un material y retornar el DTO', async () => {
      ...
      const result = await controller.create({ name: 'Cartón', currentPrice: 2 });

      expect(result.id).toBe('m-1');
      expect(result.name).toBe('Cartón');
      expect(mockCreate.execute).toHaveBeenCalledWith({ name: 'Cartón', currentPrice: 2 });
    });
  });
  ...
  describe('changePrice', () => {
    it('debería cambiar el precio del material', async () => {
      const updated = new Material({
        id: 'm-1', name: 'Cartón', currentPrice: 5, active: true,
        createdAt: new Date(), updatedAt: new Date(),
      });
      mockChangePrice.execute.mockResolvedValue(updated);

      const result = await controller.changePrice({ id: 'm-1' }, { currentPrice: 5 });
  ...
  describe('deactivate', () => {
    it('debería desactivar el material', async () => {
      const deactivated = new Material({
        id: 'm-1', name: 'Cartón', currentPrice: 2, active: false,
        createdAt: new Date(), updatedAt: new Date(),
      });
