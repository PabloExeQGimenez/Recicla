...
describe('SolicitudPagoController', () => {
  ...
  const solicitud = new SolicitudPago({
    id: 'sp-1', from: new Date('2026-01-01'), to: new Date('2026-01-31'),
    status: SolicitudPagoStatus.PAYMENT_REQUESTED, createdAt: new Date(),
  });
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: ExcludePesajesFromSolicitudPagoUseCase, useValue: mocks.exclude },
  ...
  describe('findAll', () => {
    it('debería retornar solicitudes paginadas', async () => {
      mocks.getAll.execute.mockResolvedValue({
        data: [{ id: 'sp-1', from: new Date(), to: new Date(), status: 'PAYMENT_REQUESTED', createdAt: new Date(), totalAmount: 100, itemsCount: 5 }],
  ...
  describe('markAsPaid', () => {
    it('debería marcar la solicitud como pagada', async () => {
      const paid = new SolicitudPago({
        id: 'sp-1', from: new Date('2026-01-01'), to: new Date('2026-01-31'),
        status: SolicitudPagoStatus.PAID, createdAt: new Date(),
      });
  ...
  describe('excludePesajes', () => {
    it('debería excluir pesajes de la solicitud', async () => {
      ...
      const result = await controller.excludePesajes({ id: 'sp-1' }, { pesajeIds: ['p-1'] });
    ...
    it('debería retornar null si la solicitud se elimina', async () => {
      ...
      const result = await controller.excludePesajes({ id: 'sp-1' }, { pesajeIds: ['p-1', 'p-2'] });
  ...
  describe('export', () => {
    it('debería enviar el buffer con los headers correctos', async () => {
      ...
      expect(mockResponse.set).toHaveBeenCalledWith({
        ...
        'Content-Disposition': 'attachment; filename="solicitud-pago-sp-1.xlsx"',
      });
