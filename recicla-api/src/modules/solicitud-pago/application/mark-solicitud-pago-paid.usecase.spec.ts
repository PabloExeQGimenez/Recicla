...
describe('MarkSolicitudPagoPaidUseCase', () => {
  ...
  const makePesaje = (id: string, status: PesajeStatus = PesajeStatus.PAYMENT_REQUESTED): Pesaje =>
    new Pesaje({
      id,
      recuperadorId: 'rec-1',
      status,
      items: [new PesajeItem({ materialId: 'mat-1', weight: 10, pricePerKgAtMoment: 2 })],
      date: new Date(),
      createdAt: new Date(),
      updatedAt: new Date(),
    });
  ...
  beforeEach(() => {
    ...
    mockPesajeRepo = { removeFromPaymentRequest: jest.fn(), markAsPaid: jest.fn() } as any;
    useCase = new MarkSolicitudPagoPaidUseCase(mockSolicitudRepo, mockPesajeRepo);
  });
  ...
  it('debería lanzar BadRequestException si todos los pesajes son excluidos', async () => {
    ...
    await expect(
      useCase.execute('sp-1', ['p-1', 'p-2']),
    ).rejects.toThrow(BadRequestException);
    await expect(
      useCase.execute('sp-1', ['p-1', 'p-2']),
    ).rejects.toThrow('todos los pesajes fueron excluidos');
  });
  ...
  it('debería excluir pesajes y marcar el resto como pagados', async () => {
    ...
    expect(result.status).toBe(SolicitudPagoStatus.PAID);
    expect(mockPesajeRepo.removeFromPaymentRequest).toHaveBeenCalledWith(['p-2']);
